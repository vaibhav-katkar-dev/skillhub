...
const simulationCouponSchema = new mongoose.Schema({
  code: {
    type: String,
    required: true,
    unique: true,
  ...
  simIds: {
    // Array of simulation IDs this coupon applies to. Empty array means ALL simulations.
    type: [String],
    default: [],
  },
  maxUsageLimit: {
...
simulationCouponSchema.index({ isActive: 1, validUntil: 1 });

/** Returns true only if this coupon is currently valid for use */
simulationCouponSchema.methods.isValid = function () {
 ...
 */
simulationCouponSchema.methods.applyDiscount = function (originalPaise) {
...
export default mongoose.model('SimulationCoupon', simulationCouponSchema);
